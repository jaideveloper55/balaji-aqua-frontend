...
import { Drawer, Tag, Divider, Modal } from "antd";
import {
...
const InvoiceDetailDrawer: React.FC<Props> = ({
  ...
  const handleCancelClick = () => {
    if (!invoice || !onCancel) return;
    Modal.confirm({
      title: `Cancel invoice ${invoice.invoiceNo}?`,
      icon: <HiOutlineExclamationCircle className="w-5 h-5 text-red-500" />,
      content:
        "This will restore stock and reverse the customer's outstanding balance. This action cannot be undone.",
      okText: "Yes, cancel invoice",
      okType: "danger",
      cancelText: "Keep invoice",
      onOk: () => onCancel(invoice),
    });
  };
          ...
          <div className="space-y-2">
            ...
            {canCancel && (
              <button
                ...
                className="w-full py-2.5 rounded-xl border border-red-200 text-red-600 hover:bg-red-50 font-semibold text-[13px] flex items-center justify-center gap-2"
              >
            ...
            {invoice.status === "Cancelled" && (
              <div className="w-full py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-gray-500 font-medium text-[12px] flex items-center justify-center gap-2">
                <HiOutlineBan className="w-4 h-4" /> This invoice has been
                cancelled
              </div>
            )}
          </div>
