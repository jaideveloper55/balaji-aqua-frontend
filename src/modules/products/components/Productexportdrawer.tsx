...
type ReportType = "products" | "categories";
...
const REPORTS: {
...
}[] = [
  {
    key: "products",
    title: "Product List",
    description: "All products with pricing, stock & category info",
  ...
  },
  {
    ...
    description: "Grouped view of products sorted by category",
...
const ProductExportDrawer: React.FC<Props> = ({
  open,
  onClose,
  defaultReportType = "products",
  products,
  productCount,
}) => {
  ...
  const recordCount = productCount;

  const handleExport = async () => {
    if (recordCount === 0) {
      message.warning("Nothing to export — no products loaded");
      return;
    }
    ...
    try {
      if (format === "pdf") {
        // For "by category" — sort by category before passing
        const sorted =
          reportType === "categories"
            ...
            : products;
        await generateProductsPDF(sorted);
      } else {
...
export default ProductExportDrawer;
